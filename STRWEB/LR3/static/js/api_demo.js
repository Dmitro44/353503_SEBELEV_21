        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
                ...
                switch (error.code) {
                    ...
                    case error.TIMEOUT:
                        errorMessage =
                            "Истекло время ожидания запроса на получение координат.";
                        break;
                }
    ...
    batteryBtn.addEventListener("click", () => {
        if ("getBattery" in navigator) {
            navigator.getBattery().then((battery) => {
                ...
                batteryResult.innerHTML +=
                    "<br><i>(Статус будет обновляться автоматически)</i>";
            });
        } else {
            batteryResult.textContent =
                "Battery Status API не поддерживается вашим браузером.";
        }
    });
