    ...
    geoBtn.addEventListener('click', () => {
        ...
        navigator.permissions.query({ name: 'geolocation' }).then(permissionStatus => {
            if (permissionStatus.state === 'granted') {
                ...
                geoResult.innerHTML = '<strong>Ошибка: Доступ к геолокации заблокирован.</strong><br>Пожалуйста, измените настройки разрешений для этого сайта в вашем браузере (обычно это делается по клику на иконку замка в адресной строке).';
            }
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                geoResult.innerHTML = `
                    ...
                    <a href="https://www.openstreetmap.org/#map=18/${latitude}/${longitude}" target="_blank" rel="noopener noreferrer">Показать на карте OpenStreetMap</a>
                `;
            },
            (error) => {
                ...
                switch (error.code) {
                    ...
                    case error.POSITION_UNAVAILABLE:
                        errorMessage = '<strong>Информация о местоположении недоступна.</strong><br>Возможные причины:<ul><li>На устройстве отключен GPS/службы геолокации.</li><li>Отсутствует подключение к сети или слабый сигнал.</li><li>Браузер не может получить доступ к службам геолокации ОС.</li></ul>';
                        break;
                    case error.TIMEOUT:
