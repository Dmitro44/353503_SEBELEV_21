document.addEventListener('DOMContentLoaded', () => {
    ...
    const createFormElement = (state) => {
        ...
        formElement.querySelector('[data-attr="multiple"]').checked = state.multiple;
        formElement.querySelector('[data-attr="required"]').checked = state.required;
        formElement.querySelector('[data-attr="disabled"]').checked = state.disabled;
    ...
    const loadState = () => {
        ...
        if (savedState) {
            try {
                ...
                formContainer.innerHTML = ''; // Clear existing content before loading
                state.forEach(elementState => createFormElement(elementState));
            } catch (e) {
                console.error("Error parsing form generator state from localStorage", e);
                localStorage.removeItem('formGeneratorState'); // Clear corrupted state
            }
        }
    };
    ...
    const attachEventListenersToForm = (formElement) => {
        const selectElement = formElement.querySelector('select');
        const attributeControls = formElement.querySelectorAll('.attribute-control');
    ...
    addElementCheckbox.addEventListener('change', () => {
        if (addElementCheckbox.checked) {
            createFormElement({ name: '', size: 1, multiple: false, required: false, disabled: false });
            saveState();
