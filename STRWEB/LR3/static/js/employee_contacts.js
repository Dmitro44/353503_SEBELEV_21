document.addEventListener("DOMContentLoaded", () => {
    ...
    function handleBonus() {
        if (checkedContactIds.size === 0) {
            bonusMessageContainer.innerHTML =
                "<p>Пожалуйста, выберите сотрудников для премирования.</p>";
            bonusMessageContainer.style.display = "block";
            return;
        }

        const employeeNames = Array.from(checkedContactIds)
            .map((id) => {
    ...
    function validatePhone(phone) {
        ...
        const validOperatorCodes = ["29", "44", "33", "25"];
        ...
        if (hasPlus) {
            ...
            if (digits.startsWith("80") && digits.length === 11) {
                const operatorCode = digits.substring(2, 4);
                return validOperatorCodes.includes(operatorCode);
            }
        }
